    ...
    def pd(self, d_iq, vco):
        if(self.mode == 'costas'):
            if self.opf=='R':
                ...
                self.temp(vco*Fxp(d_iq.real).like(DATA))
                self.phase_difference(self.temp.real * self.temp.imag)
            ...
            if self.opf=='R':
                ...
                self.phase_difference((Fxp(np.conjugate(d_iq)).like(DATA)*vco).imag)
        else:
            if self.opf=='R':
                self.phase_difference = (np.conjugate(d_iq)*vco).imag
            else:
...
class PLL(object):
    def __init__(self,fs,fc, lf_gain, lf_bandwidth, lf_damping,lf_delay = 1,pd_type='pll',opf='R'):
        ...
        if opf == 'Q':
            self.phase_difference = Fxp(self.phase_difference).like(DATA)
            self.phase_estimate = Fxp(self.phase_estimate).like(CANGLE)
            self.vco = Fxp(self.vco).like(DATA)
            self.omega = Fxp(self.omega).like(CCARRY)
    ...
    def update_phase_estimate(self):
        ...
        if self.opf == 'R':
            ...
            self.vco = np.exp(1j*(2*np.pi*self.omega*(self.n)+self.phase_estimate))
        else:
            self.phase_estimate(self.phase_estimate + self.loop_reg[self.delay])
            self.vco(np.exp(1j*(2*np.pi*self.omega.get_val()*self.n+ self.phase_estimate)))
...
class COMB(object):
    def __init__(self,fs,fc,fm, lf_gain, lf_bandwidth, lf_damping, lf_delay=1,opf='R'):
        ...
        if opf == 'Q':
            ...
            self.phase_estimate = Fxp(self.phase_estimate).like(ADDER)
            self.vco = Fxp(self.vco).like(DATA)
