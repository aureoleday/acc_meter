...
class LoopFilter(object):
    def __init__(self, gain, Bn, zeta,opf='R'):
        ...
        self.opf = opf
        if opf=='Q':
            self.kp = Fxp(self.kp,dtype='S1.18')
            self.ki = Fxp(self.ki,dtype='S1.18')
            self.integrater = Fxp(0,dtype='S8.18')
            self.lf_out = Fxp(0,dtype='S2.18')
        print("kp:%f, ki:%f" %(self.kp,self.ki))
        
    def advance_filter(self, phase_difference,dt='S2.18'):
        if self.opf=='R':
            ...
            self.lf_out(self.integrater + self.kp*phase_difference)
            return self.lf_out.get_val()

class PhaseDetector(object):
    def __init__(self,mode = 'pll',opf='R'):
        self.phase_difference = 0
        if opf=='Q':
            self.phase_difference = Fxp(self.phase_difference,dtype='S1.15')
        self.mode = mode;
        self.monitor = 0
        self.pdf0 = 0
        self.pdf1 = 0
    ...
    def pd(self, d_iq, vco,dt='S1.15'):
        if(self.mode == 'costas'):
            ...
            if self.opf=='R':
                ...
                temp = Fxp(vco,dtype='S1.15')*Fxp(d_iq.real,dtype='S1.15')
                self.phase_difference(temp.real * temp.imag)
            ...
            if self.opf=='R':
                ...
                self.phase_difference((Fxp(np.conjugate(d_iq),dtype=dt)*Fxp(vco,dtype=dt)).imag)
        else:
            if self.opf=='R':
                self.phase_difference = (np.conjugate(d_iq)*vco).imag
            else:
        ...
        if self.opf == 'R':
            ...
            # return Fxp(self.phase_difference,dtype=dt).get_val()
            return self.phase_difference.get_val()
...
class PLL(object):
    def __init__(self,fs,fc, lf_gain, lf_bandwidth, lf_damping,lf_delay = 1,pd_type='pll',opf='R'):
        ...
        if opf == 'Q':
            self.phase_difference = Fxp(0,dtype='S1.15')
            self.phase_estimate = Fxp(self.phase_estimate,dtype='S1.15')
            self.vco = Fxp(self.vco,dtype='S1.15')
            self.omega = Fxp(self.omega,dtype='S1.15')
    ...
    def update_phase_estimate(self,dt='S1.15'):
        self.n += 1        
        if self.opf == 'R':
            self.phase_estimate += self.loop_reg[self.delay]
            self.vco = np.exp(1j*(self.omega*(self.n)+self.phase_estimate))
        else:
            self.phase_estimate(self.phase_estimate + self.loop_reg[self.delay])
            self.vco(np.exp(1j*((self.omega.get_val()*(self.n))+ self.phase_estimate.get_val())))
        self.loop_reg[1:] = self.loop_reg[:-1]
        self.loop_reg[0] = self.loop_filter.lf_out

    def step(self, d_iq,dt='S1.15'):
...
class COMB(object):
    def __init__(self,fs,fc,fm, lf_gain, lf_bandwidth, lf_damping, lf_delay=1,opf='R'):
        ...
        self.phase_difference = 0.0
        if opf == 'Q':
            self.phase_difference = Fxp(0,dtype='S1.18')
            self.phase_estimate = Fxp(self.phase_estimate,dtype='S4.18')
            self.vco = Fxp(self.vco,dtype='S1.15')
            self.demod = Fxp(self.demod,dtype='S1.18')
        self.loop_filter = LoopFilter(lf_gain, lf_bandwidth, lf_damping,opf=opf)
        self.loop_reg = np.zeros(32)
        self.N = 16384
    ...
    def update_phase_estimate(self):
        ...
        if self.opf == 'R':
            ...
            self.phase_estimate((self.phase_estimate + self.loop_reg[self.delay])%(2*np.pi))
            self.rad = self.phase_estimate.get_val()
        # self.rad = self.phase_estimate
...
def comb_tb():
    pll = COMB(fs,fc,fm,Kd, Bn, Zeta,lf_delay=1,opf='Q')
