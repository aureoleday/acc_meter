# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 15:50:00 2019
...
def corr(seq1,seq2):
    corr_arr = []
    s1 = (np.array(seq1)-0.5)*2
    s2 = (np.array(seq2)-0.5)*2
    if len(seq1) != len(seq2):
        raise AttributeError("Can not calc corr with different size.\n")
    else:
        temp = np.array(s2)
        for i in range(len(s1)):            
            temp = np.hstack([temp[-1],temp[:-1]])
            corr_arr.append(np.dot(s1,temp))
    return corr_arr
...
#    poly1 = [12, 10, 5, 4, 0]
#    poly2 = [12,  8, 6, 5, 0]
    init = np.zeros(6).astype(int).tolist()
    init[-1] = 1
    poly1 = [6, 5,  0]
    poly2 = [6, 1,  0]
