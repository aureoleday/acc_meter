...
class COMB(object):
    ...
    def step(self, d_iq,dt='S1.15'):
        ...
        if self.opf == 'R':
            ...
            self.phase_difference = Fxp(self.phase_detector.pd(d_iq,self.vco),dtype=dt).get_val()
        self.costas.step(d_iq)
        self.demod = self.costas.vco.real*self.phase_difference
        if self.opf == 'R':
            self.demod = self.costas.vco.real*self.phase_difference
        else:
            self.demod = Fxp(self.costas.vco.real*self.phase_difference,dtype=dt).get_val()
