...
def main(argv):
    ...
    if rdc_dsrc == '1':
        src = pd.read_csv("rdc_src.csv")
        N = src.size
        print("rdc_data source: [external]")
    else:
        src = sig_gen()
        N = cfg.com.N
        print("rdc_data source: [internal]")
        
    ts = np.arange(N-1)/cfg.com.fs    

    for i in range(0, N - 1):
        if rdc_dsrc == '1':
            t2 = complex(src.iloc[:,-1][i])
            t1 = 0
        else:
            t1,t2 = next(src)
    ...
    # src = pd.DataFrame(dic['iq_dr'])
    # src.to_csv('rdc_src.csv',index=False,header=True)
    df = pd.DataFrame(dic)
    df.to_csv('rdc_out.csv')
    ...
    err_arr = np.array(dic['v'][int(N*0.5):])
