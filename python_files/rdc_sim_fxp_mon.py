...
class COMB(object):
    def __init__(self,
        ...
        self.costas_q = PLL(fs,fc,clf_gain, clf_bandwidth, clf_damping,lf_delay,pd_type='costas',opf=opf)
    ...
    def step(self, d_iq):
        ...
        # self.loop_filter.advance_filter(self.lpf_dm.filt([self.demod]))
        self.update_phase_estimate()
...
def comb_tb():
    ...
    ax = plt.subplot(312)
    ax.plot(cfg.com.ts,(np.array(dic['angle_c'])+np.pi)%(2*np.pi)-np.pi,label='phase_c') 
    ax.plot(cfg.com.ts,dic['acc'],label='accelleration')
    ax.plot(cfg.com.ts,dic['v'],label='velocity')
    ...
    plt.grid(1)
    ax = plt.subplot(212)
    ax.plot(cfg.com.ts,list(map(lambda x:x.real,dic['mon_iq'])),label='cos')
    ax.plot(cfg.com.ts,list(map(lambda x:x.imag,dic['mon_iq'])),label='sin')
    ax.plot(cfg.com.ts,dic['demon'],label='demon')
    plt.legend()
    plt.show()
