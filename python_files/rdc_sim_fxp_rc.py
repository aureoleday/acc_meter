...
class PhaseDetector(object):
    ...
    def pd(self, d_iq, vco):
        if(self.mode == 'costas'):
            ...
            if self.opf=='R':
                self.phase_difference = d_iq.real*vco.imag - dpsc*d_iq.imag*vco.real
...
def comb_tb():
    ...
    for i in range(0, N - 1):
        if(fm+a/fs*i)<fmax:            
            ...
            if i>step_thresh:
                d_iq = np.exp(1j*(2*np.pi*(fm/fs*i+0.5*a/fs/fs*i*i)+theta+step_value))
            else:
        ...
        # iq_dr.append(tt+complex(AGWN(0.5,SNR),AGWN(0.5,SNR)))
        iq_dr.append(tt)
        # iq_dr[-1] = my_iir.filt(iq_dr[-1])
        ...
        mix.append(complex(np.sin(2*np.pi*fc/fs*(1+fd)*i + phi)*iq_dr[-1].real,np.sin(2*np.pi*fc/fs*(1+fd)*i + phi+ps)*iq_dr[-1].imag))
        # mix.append(np.sin(2*np.pi*fc/fs*(1+fd)*i + phi)*iq_dr[-1])
        mix[-1] = mix[-1]+complex(AGWN(0.5,SNR),AGWN(0.5,SNR))+dc
        # mix.append(np.sin(2*np.pi*fc/fs*(1+fd)*i + phi)*iq_dr[-1])
        mix_hpf.append(my_iir.filt(mix[-1]))
        pll.step(mix[-1] if iir_bypass==1 else mix_hpf[-1])
        iq_d.append(d_iq)
        ...
        # ef_c.append(pll.demod.get_val() if usr_opf=='Q' else pll.demodf)
        ef_c.append(pll.demod.get_val() if usr_opf=='Q' else pll.demod)
        # acc.append(pll.costas.loop_filter.integrater.get_val() if usr_opf=='Q' else pll.costas.loop_filter.integrater)
    ...
    # ax.plot(ts,acc,label='integrator')
    ...
    # print("enob: ",np.log(2*np.pi/(abs(np.mean(err_arr))+std)/np.log(2)-1.76))
